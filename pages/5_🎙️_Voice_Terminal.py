...
with col1:
    st.markdown("<h3 style='font-family: Orbitron; color: #00f0ff; text-align: center; font-size:1.2rem; letter-spacing:1px;'>CORE INTERACTION ORB</h3>", unsafe_allow_html=True)
    ...
    html_component_c    # Safely swap out text data without utilizing Python f-string compiling mechanisms
    html_component_code = html_component_raw.replace("__SPEECH_PAYLOAD_PLACEHOLDER__", speech_payload)
    st.components.v1.html(html_component_code, height=420)
    
    st.markdown("""
    <div style="text-align: center; color: rgba(255,255,255,0.4); font-size: 0.85rem; margin-top:-10px;">
        💡 <b>Click the central Orb</b> to pause SAnA's automated microphone listening sequence or toggle her offline.
    </div>
    """, unsafe_allow_html=True)

with col2:
    st.markdown("<h3 style='font-family: Orbitron; color: #bd00ff; font-size:1.2rem; letter-spacing:1px;'>REAL-TIME TRANSMISSION DATA</h3>", unsafe_allow_html=True)
    
    # Generate live looping container representation for logs
    log_html = '<div class="console-box">'
    for entry in reversed(st.session_state.log):
        tag_class = "tag-user" if entry["sender"] == "User" else "tag-sana"
        log_html += f"""
        <div class="log-line">
            <span class="log-time">[{entry["time"]}]</span>
            <span class="log-tag {tag_class}">{entry["sender"].upper()}</span>
            <span style="color: #e0e0ff;">{entry["text"]}</span>
        </div>
        """
    if not st.session_state.log:
        log_html += '<div class="log-line" style="color: grey;">Matrix console logs idle. Syncing core terminal channels...</div>'
    log_html += '</div>'
    
    st.markdown(log_html, unsafe_allow_html=True)
    
    # Quick Text Fallback Terminal Input Engine
    st.markdown("<div style='margin-top: 15px;'></div>", unsafe_allow_html=True)
    text_input = st.text_input("Manual Command Link Overlay:", placeholder="Type a message here if your mic environment is noisy...", label_visibility="collapsed")
    if text_input:
        user_text = text_input.strip()
        st.session_state.log.append({"sender": "User", "text": user_text, "time": time.strftime("%H:%M:%S")})
        sana_text_resp = generate_sana_response(user_text)
        st.session_state.log.append({"sender": "SAnA", "text": sana_text_resp, "time": time.strftime("%H:%M:%S")})
        st.session_state["sana_speech_queue"] = sana_text_resp
        st.rerun()

# --- ATTACH SELF-CONTAINED SOURCE CODE PACKAGE EXPORTER ---
try:
    ...
    st.sidebar.download_button(
        label="📥 Download Terminal Source Package",
        data=current_file_source_code,
        file_name="5_🎙️_Voice_Terminal.py",
        mime="text/x-python",
